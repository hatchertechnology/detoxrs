...
| Stage independence seam (C §1.4)   | acknowledges the property is vacuous early, no seam                    | not raised                                                          | **defect: every stage a substitutable `&str -> String` in a list**    | **Accepted in substance, rejected in mechanism**         | The gap is real. But stages 12 and 13 do not have that signature (12 takes stem/ext/limits, 13 is a loop over 3/9/10/12 — see the note below), so a uniform function list is a false abstraction. A stage mask over the linear stages, plus direct tests for 12/13, is smaller and honest. See §5.2. |
...
over: stage 12 takes `(stem, ext, limits)` and stage 13 is a bounded loop **over** stages 3/9/10/12
(corrected from 9/10/11 during WP1-3: a deletion in stage 4 or 7 can join a base to a combining
mark, so normalization must be re-applied — found by the Idempotence property, see §12),
...
trim (10) → split_extension + truncate (12) → finalize (13): re-run 3/9/10/12 to a bounded fixed point
