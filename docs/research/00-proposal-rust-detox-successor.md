...
`Outcome` fields above are the eventual v1.0 shape; the implementation plan's M1 carries only
`{ text, truncated }`, because `stages` exists for `-vv` (M2) and `notes` accumulates variants that
later milestones introduce. `docs/plans/unified-draft-plan.md` §7.2 is authoritative for what any
given milestone actually builds.
