...
data corruption (re-interpreting bytes as Latin-1) is *structurally impossible* because legacy
...
*Falsifier: any PR that adds a `sequence`, `filter`, `pipeline = [...]`, or external character
table is rejected on sight.*
...
*Falsifier: a PR adding `--encoding <x>` for input interpretation. `--legacy-encoding` (which
only names the fallback for already-invalid-UTF-8 bytes) is the sole permitted exception and is
documented as a repair hint, not an input declaration.*
...
*Falsifier: any `--overwrite`/`--force-replace` flag, any code path that can call a
clobbering rename, any collision handled by `unwrap()`/silent skip.*
...
*Falsifier: a PR that makes any lossy or taste-driven transform on by default.*
...
`--dry-run` "the most important option to learn" (doc 02 theme 7). detox itself does *not*
default to dry-run (confirmed from the local man page, doc 07 row 8c).
*Falsifier: any code path where a bare `detoxrs <path>` mutates the filesystem, unless the
user's own config set `exec = true`.*
...
*Falsifier: any `.to_str().unwrap()`, `to_string_lossy()` outside display/log code, or
`String`-typed filename in a non-display signature.*
...
doc 07 row 7b).
*Falsifier: a PR that adds a direct dependency without (a) a line in `docs/deps.md` saying why
50 lines of our own code will not do, and (b) a last-release date inside 12 months.*
...
and a y/n loop is the *weakest* of the interactive designs in the research: `qmv`'s
...
length limits of *this* filesystem) lives in the planner (§5). That split is what makes the
...
| 3 | `normalize` | NFC | Unicode normalization of the output name. Comparison inside the planner is *always* NFC regardless of this setting. |
...
| 9 | `collapse` | on | Collapse runs of the *same* separator character to one. Do **not** merge runs of different separators. Drop separators adjacent to `.`. |
...
- **1 before everything.** Nothing textual is meaningful before decode. Critically, stage 1 only
  attempts legacy decoding on input that *failed* UTF-8 validation. This makes detox's worst bug
...
- **5 before 6/7.** A user rule such as `ü -> ue` must be able to run *before* transliteration
...
Two things we deliberately do not do. (a) Mojibake repair of *valid* UTF-8 (ftfy-style
...
as two, but also that *rewriting* names is a policy choice with real cost (rclone #1472, #4228:
...
Collapse runs of the *same* separator character only. `a__b` -> `a_b`. `a--b` -> `a-b`.
...
| Character *tables* as external files | P1/P2. `.tbl` files are the config-complexity theme's root cause (doc 02 theme 1). `--keep`/`--strip`/`[[rule]]` cover the real asks (#7, #84, #117, #121, #124). |
| Which encoding to decode *valid* UTF-8 as | P2. There is no such option; this is the whole point. |
...
| Case | Where | Asserts |
|---|---|---|
| Case-only rename | macOS case-insensitive APFS image, macOS case-sensitive APFS image, Linux ext4, Linux tmpfs | Succeeds in one syscall via `rename_case_only`, and is *not* misreported as a collision. This is the doc 06 Test 3 refutation, made permanent. |
...
namespace is *populated*, not that it is unusable, and that a same-family name will be read as
...
blocked axo.dev's own rebrand, so a compound like `detoxrs` is *lower* risk than the invented
...
- No `detoxrc` parsing, ever, in any form. This is option 3 from doc 04 §6A. Reason: the config
  grammar *is* the thing the mandate rejects, and doc 05 records that even detox's own merge
...
- No flag aliasing (doc 04 §6B option 2). `-n` happens to mean the same thing in both, which is
  ...
  Notably `-f` is *not* accepted, because it means "config file" in detox and "force" in rnr, and
...
- **f2** (Go, v2.2.2 2025-11-10, ~2.4k stars, actively maintained, doc 06 row 6b, doc 07 row 1a)
  ...
  variables, counters, CSV batch mode. `detoxrs` supplies the *policy* and you supply nothing.
...
direct fetch failed in both research passes and rests on a search-summarized snapshot), *and* its
...
*Closes with:* `cargo search`/crates.io API for `detoxrs`, GitHub org+repo check, Debian
...
*Closes with:* a matrix run on real Linux: ext4, xfs, btrfs, tmpfs, overlayfs, NFSv4, CIFS, ZFS,
...
the *normal* path, which decides whether the fallback deserves more engineering than a warning.
...
*Closes with:* on real Windows 11 and real Windows 10, attempt to create `con.txt`, `nul.txt`,
...
*Closes with:* the same four-character-class binary search doc 06 ran on APFS (ASCII, precomposed
...
*Closes with:* the case-only rename test against SMB (macOS and Linux clients), NFSv3/v4, and an
...
*Closes with:* on Linux tmpfs/ext4, create names from a real corpus of CP1252, Latin-1, KOI8-R,
...
*Closes with:* running the planner in report-only mode over large real macOS trees (a Photos
...
*Closes with:* user feedback on the v0.1 preview output, plus counting how many conflicts a real
...
*Closes with:* a `criterion` benchmark of walk + plan + apply over 200k entries at 1, 2, 4, and 8
...
*Closes with:* a direct query to Repology's API from a machine with working DNS, before any
...
| Dry-run default | doc 04 §1 (f2, rnr) | doc 07 rows 1a/1c confirm both; row 8c confirms detox does *not* |
