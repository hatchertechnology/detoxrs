...
`filename` is a *hard* reference: if given, no search/fallback happens — a parse failure there is
...
| `cp1252-only` | `iso8859_1(builtin=cp1252)` only (note: this is the `iso8859_1` *filter*, just pointed at the `cp1252` builtin table — there is no separate CP-1252 filter type) |
...
name was requested and no sequence named `default` exists, it silently uses the *first* sequence
...
2. Unpack the lead byte's payload bits, then consume `width-1` continuation bytes, shifting 6 bits
   ...
   the sequence, emit `_`, and resume scanning from the *next* byte after the point of failure
...
— sized per raw input *byte*, not per decoded character, which is always sufficient since a
...
2. Build a "search" set: `"-_"` normally, or `".-_"` if `remove_trailing` is set
   ...
   note the *order* of this string is the precedence order used in step 3.
3. Walk the (already-stripped) string. Any char in the search set is *not* copied immediately;
   instead its position in the search string is compared (`seek < current`) against a
   previously-held candidate for the current run, and the *earliest-appearing-in-`search`* one wins
...
4. On the first non-search-set char after a run, flush the winning representative char, then copy
   ...
   [src/clean_string.c:236-238](https://github.com/dharple/detox/blob/0a8e212/src/clean_string.c#L236-L238) — i.e. trailing separator runs are *not* deleted, they are collapsed to one char, same as internal runs.
...
(dash wins) and `.` runs are *not* touched (not in search set without remove_trailing) →
...
- **Lookback-for-double-extension**: starting just before the last `.`, walk backward up to 5
  characters looking for *another* `.` (e.g. to catch `.tar.gz`); if found within that 5-char
  window, that earlier `.` becomes the new "extension" start instead
  [src/clean_string.c:285-295](https://github.com/dharple/detox/blob/0a8e212/src/clean_string.c#L285-L295). 5 is a hardcoded constant, not configurable, and it counts characters *between* the
...
- If the (possibly extended) extension's length is `>= max_length`, give up entirely and return the
  **original, untruncated** filename with a stderr warning
  [src/clean_string.c:297-302](https://github.com/dharple/detox/blob/0a8e212/src/clean_string.c#L297-L302) — this is a case where the filter can produce output *longer* than `max_length`.
...
- `#` as the *first character of the line* → whole line ignored (comment). Note: this is a
  ...
  data line is *not* stripped either — the value token is captured via `%s`/`%[^"]` which stops at
...
- `start` (BASE_STATE only, case-insensitive `strncasecmp(...,5)`): with nothing after it → enter
  ...
  is compared case-insensitively against the current `LC_CTYPE` locale string (`setlocale(LC_CTYPE, "")`); if it's a case-insensitive *prefix match* (`strncasecmp(parsed, system_ctype, strlen(parsed))`)
  ...
  `english`, etc. — any locale string that merely *starts with* `en`.
...
- A line inside `INSIDE_STATE` that is *not* a valid `%i`-scannable code is checked for the literal
...
- **No `\uXXXX`/`\xXX` escape interpretation happens at table-parse time at all.** The value string
  captured by `sscanf` (whether quoted or bare) is stored *verbatim* as bytes — `"¡"` in a
...
| `table/cp1252.tbl` | 64 | commented out (none) | Only covers 0x80-0x9F (the CP-1252-specific block that differs from Latin-1) — meant to run *before* `iso8859_1(builtin=iso8859_1)` in a chain so 0xA0-0xFF still gets handled by the latter. [table/cp1252.tbl](https://github.com/dharple/detox/blob/0a8e212/table/cp1252.tbl) |
...
- **`.tbl` file `\u`/`\x` escapes do not work at runtime**, only at compile-time for the four
  ...
  `\uXXXX` would be a deliberate *behavioral improvement/divergence* from upstream detox, not parity;
