...
- With `-L` alone: prints `available sequences:` then each sequence name indented, marking the active one with `(*)` — [src/config_file_dump.c:22-38](https://github.com/dharple/detox/blob/0a8e212/src/config_file_dump.c#L22-L38).
- With `-L -v`: switches to a different, unindented format — `sequence name: X`, `source file: ...`, then one line per filter/cleaner and its properties (e.g. `remove trailing: yes/no` for wipeup, `length: N` for max_length, `builtin table:`/`translation table:`) — [src/config_file_dump.c:24-73](https://github.com/dharple/detox/blob/0a8e212/src/config_file_dump.c#L24-L73).
...
- **[UNVERIFIED]** Whether `DETOX_SEQUENCE` is documented anywhere for end users: it does not appear in `man/detox.1` or `man/inline-detox.1`; it is referenced in `man/detoxrc.5` and `CHANGELOG.md` per a repo grep, but I did not read `man/detoxrc.5` in full to confirm the exact wording there. Evidence that would settle it: full read of `man/detoxrc.5`.
...
| `detox: no config file to work with`, `detox: no sequence to work with`, `detox: unable to open: X`, `detox: an error occurred while parsing command line arguments`, `unknown option: %c` (default case) | stderr | [src/detox.c:39,46,77](https://github.com/dharple/detox/blob/0a8e212/src/detox.c#L39), [src/config_file.c:52](https://github.com/dharple/detox/blob/0a8e212/src/config_file.c#L52), [src/parse_options.c:218](https://github.com/dharple/detox/blob/0a8e212/src/parse_options.c#L218) |
...
| `DETOX_SEQUENCE` env var | Absent from detox.1/inline-detox.1; referenced in detoxrc.5 (not fully read) | Present, read at options-init time, overridable by `-s` | **[UNVERIFIED]** full documentation status — read `man/detoxrc.5` fully to settle |
| `--version` long option | Neither man page lists one (both only show `-V`) | Confirmed absent from `longopts[]` | **Consistent** — no discrepancy |

## Still [UNVERIFIED]

- Exact wording of `DETOX_SEQUENCE` in `man/detoxrc.5` — not read in full. Settle by reading that file.
...
- Whether any code path makes `parse_options_getopt` return `NULL`, making the `src/detox.c:38-41` check reachable — settle by reading `src/wrapped.c`'s `wrapped_malloc`.
